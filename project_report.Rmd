...
gov_sample <- gov[sample(nrow(gov), 5000), ]
...
plot_euclidean_distances <- function(data1) {
  ggplot2::ggplot(data1, ggplot2::aes(x=dist)) +
    ggplot2::geom_histogram(fill="#579ECF", colour="black", binwidth=1)
}
...
```{r}
print(summary(same_group_2))

print(summary(different_group))
```


The results I obtained in the summary table seem off. I think that might have to do with how I sampled the spans, without regards for the distribution of languages in the original data. The number of measures is also very different because of the way I measured the distance between every pair of point( some results between english/naija and naija/english are therefore duplicated). I have decided not to use these measurements (which i added a bit too late) in my analysis, as i do not think I can rely on them.

### Conclusion

Based on the data analysis I tried to propose, the most interesting variable to study seemed to be span_dep, which means that the dependent of verbal-governors allowed us to separate both groups of observation more easily than the span_gov variable. In order to observe this I used the Wasserstein metric which allowed me to measure the divergence in the probability distribution of spans for both languages. In the future, I would to investigate other measures, such as the Euclidean distance, and observe the influence of other variables (for example the type of the dependencies considered, the part-of-speech of preceding and following tokens, both informations that i had hoped to measure the impact of when i wrote the proposal for this project).